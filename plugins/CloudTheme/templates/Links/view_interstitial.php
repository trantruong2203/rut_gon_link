...
$search_keyword = ($campaign && !empty($campaign->keyword_or_url))
    ...
    : ($keywordTask ? $keywordTask->keyword : get_option('interstitial_search_keyword', 'Newgoal'));
...
<div class="container" style="padding: 20px 0;">
    ...
    <div class="row"><div class="col-md-8 col-md-offset-2"><div class="box box-primary"><div class="box-body">
        <h4 style="color: #337ab7; font-weight: bold; margin-bottom: 20px;">VUI LÒNG NHẬP MÃ ĐỂ TIẾP TỤC ĐẾN TRANG ĐÍCH</h4>
    ...
    <div class="row"><div class="col-md-8 col-md-offset-2"><div class="panel panel-default" style="border: 1px solid #ddd; border-radius: 8px;">
        <div class="panel-heading" style="background: #f5f5f5; border-radius: 8px 8px 0 0;"><h4 class="panel-title" style="font-weight: bold;"><?= h($instruction_header) ?></h4></div>
        <div class="panel-body" style="background: #fff;">
            ...
            <ol style="text-align: left; padding-left: 20px; line-height: 2;">
                ...
                <?php if ($campaign_image_1) : ?><div style="margin: 15px 0;"><img src="<?= h($campaign_image_1) ?>" alt="Hình minh họa LẤY MÃ" class="img-responsive" style="max-width: 100%; border: 1px solid #ddd; border-radius: 4px;"></div><?php endif; ?>
                <li><strong>Bước 5: Sau đó nhấn bất kỳ link nào trên website và quay lại vị trí đó để lấy mã</strong></li>
                <?php if ($campaign_image_2) : ?><div style="margin: 15px 0;"><img src="<?= h($campaign_image_2) ?>" alt="Hình minh họa click link" class="img-responsive" style="max-width: 100%; border: 1px solid #ddd; border-radius: 4px;"></div><?php endif; ?>
                <li><strong>Bước 6: Dán mã vào đây để đến trang đích</strong></li>
            </ol>
            <p style="margin-top: 20px;"><a href="<?= h($landing_url) ?>" target="_blank" class="btn btn-danger btn-lg"><i class="fa fa-external-link"></i> LẤY MÃ - Nhấn để mở trang</a></p>
        </div>
    </div></div></div>
    ...
    <div class="row"><div class="col-md-8 col-md-offset-2"><div class="panel panel-default" style="border: 1px solid #ddd; border-radius: 8px;">
        <div class="panel-body" style="background: #f9f9f9;">
            ...
            <div class="form-group"><label for="code-input-2">Nhập mã</label><input type="text" id="code-input-2" class="form-control input-lg" placeholder="NHẬP MÃ" style="text-transform: uppercase; letter-spacing: 2px; max-width: 300px;" maxlength="10" autocomplete="off"></div>
            <div class="form-group"><button type="button" id="go-submit-2" class="btn btn-success btn-lg" <?= $show_recaptcha ? 'disabled' : '' ?>>Nhấn vào đây để tiếp tục</button></div>
            <div class="form-group" style="margin-top: 20px;"><span>Bạn không lấy được mã?</span><?php if (!empty($report_url)) : ?><a href="<?= h($report_url) ?>" target="_blank" class="btn btn-danger btn-sm" style="margin-left: 10px;">Báo lỗi</a><?php else : ?><a href="<?= $this->Url->build(['controller' => 'Forms', 'action' => 'contact']) ?>" target="_blank" class="btn btn-danger btn-sm" style="margin-left: 10px;">Báo lỗi</a><?php endif; ?></div>
            <?php if (!empty($video_url)) : ?><div class="form-group"><a href="<?= h($video_url) ?>" target="_blank" class="btn btn-info"><i class="fa fa-video-camera"></i> VIDEO HƯỚNG DẪN CÁCH LẤY MÃ</a></div><?php endif; ?>
        </div>
    </div></div></div>
</div>
...
<script>
<?php if (get_option('enable_popup', 'yes') == 'yes') : ?>
$(window).on('load', function () { $(document).one("click", function (e) { if (!$(e.target).closest('#go-link').length && !$(e.target).closest('#go-submit').length && !$(e.target).closest('#go-submit-2').length) $('#go-popup').submit(); }); $('#go-popup').one("submit", function (e) { var w=screen.width-150,h=screen.height-150; window.open('about:blank','Popup_Window','width='+w+',height='+h+',left='+((screen.width-w)/2)+',top='+((screen.height-h)/2)); this.target='Popup_Window'; }); });
<?php endif; ?>
