...
<?php
$verificationStatuses = \App\Service\CampaignVerificationService::getVerificationStatuses();
$verificationStatus = (int)($campaign->verification_status ?? \App\Service\CampaignVerificationService::STATUS_UNVERIFIED);
$verificationCode = '';
$verificationScriptUrl = '';
if (!empty($campaign->verification_token)) {
    $verificationCode = \App\Service\CampaignVerificationService::generateCode($campaign->verification_token);
    $verificationScriptUrl = $this->Url->build([
        'prefix' => false,
        'controller' => 'Links',
        'action' => 'campaignVerificationScript',
        $campaign->verification_token
    ], ['fullBase' => true]);
}
?>
...
<div class="box box-primary">
    <div class="box-body">
        ...
        <legend><?= __('Website Verification') ?></legend>
        <table class="table table-hover table-striped">
            <tr>
                <td><?= __('Verification Status') ?></td>
                <td><?= h($verificationStatuses[$verificationStatus] ?? __('Unverified')) ?></td>
            </tr>
            <tr>
                <td><?= __('Last Checked') ?></td>
                <td>
                    <?= !empty($campaign->verification_checked_at) ? display_date_timezone($campaign->verification_checked_at) : __('Not checked yet') ?>
                </td>
            </tr>
            <tr>
                <td><?= __('Check Note') ?></td>
                <td><?= h($campaign->verification_note ?: __('No note')) ?></td>
            </tr>
            <tr>
                <td><?= __('Verification Token') ?></td>
                <td><code><?= h($campaign->verification_token ?: '-') ?></code></td>
            </tr>
            <tr>
                <td><?= __('Current 6-digit code') ?></td>
                <td><code><?= h($verificationCode ?: '-') ?></code></td>
            </tr>
        </table>

        <?php if (!empty($verificationScriptUrl)) : ?>
            <p><?= __('Add this script to your website source, then run verification.') ?></p>
            <pre>&lt;script async src="<?= h($verificationScriptUrl) ?>"&gt;&lt;/script&gt;</pre>
            <p><?= __('Optional fallback: place this 6-digit code in your HTML source:') ?> <code><?= h($verificationCode) ?></code></p>
        <?php endif; ?>

        <?= $this->Form->postLink(
            __('Check Verification Now'),
            ['action' => 'checkVerification', $campaign->id],
            ['confirm' => __('Run verification check now?'), 'class' => 'btn btn-info']
        ); ?>
