        ...
        <table class="table table-hover table-striped">
            ...
            <?php foreach ($pages as $page): ?>
                <tr>
                    <td><?= $page->id ?></td>
                    <td>
                        <?= $this->Html->link($page->title, ['action' => 'edit', $page->id, '?' => ['lang' => get_option('language', 'en_US')] ]); ?> <?= $this->Html->link(get_option('language', 'en_US'), ['action' => 'edit', $page->id, '?' => ['lang' => get_option('language', 'en_US')] ], ['class' => 'label label-primary']); ?><br>
                        <p>
                        <?php foreach ( get_site_languages() as $lang ) : ?>
                        <?= $this->Html->link($lang, ['action' => 'edit', $page->id, '?' => ['lang' => $lang] ], ['class' => 'label label-default']); ?>
                        <?php endforeach; ?>
                        </p>
                    </td>
                    <td><?= $page->slug ?></td>
                    ...
                    <td>
                        <?= $this->Html->link(__('View'), ['action' => 'view', $page->slug, 'prefix' => false], ['class' => 'btn btn-primary btn-xs', 'target' => '_blank']); ?>
                        
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $page->id], ['confirm' => __('Are you sure?'), 'class' => 'btn btn-danger btn-xs']);
