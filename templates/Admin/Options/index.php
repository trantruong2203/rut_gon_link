...
<div class="nav-tabs-custom">
    ...
    <div class="tab-content">
        <div role="tabpanel" id="general" class="tab-pane fade in active">
            ...
            <?php
            $localePath = defined('RESOURCES') ? RESOURCES . 'locales' : (ROOT . DS . 'resources' . DS . 'locales');
            $languages = [];
            if (is_dir($localePath)) {
                $locale = new \Cake\Filesystem\Folder($localePath);
            ...
            if (is_dir($pluginsPath)) {
                $plugins_path = new \Cake\Filesystem\Folder($pluginsPath);
        ...
        <div role="tabpanel" id="integration" class="tab-pane fade in">
            ...
            <?php if (isset($settings['final_ad_enabled'])) : ?>
            <legend><?= __('Final Ads (Smartlink / Direct Link)') ?></legend>
            <p><?= __('After user clicks confirm on the intermediate page, open Smartlink in a new tab, then redirect to the original link.') ?></p>
            <div class="row">
                <div class="col-sm-2"><?= __('Enable Final Ads') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['final_ad_enabled']['id'] . '.value', [
                        'label' => false,
                        'options' => [
                            'yes' => __('Yes'),
                            'no' => __('No')
                        ],
                        'value' => $settings['final_ad_enabled']['value'],
                        'class' => 'form-control'
                    ]);
                    ?>
                </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Final Ad URL (Smartlink / Direct Link)') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['final_ad_url']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'text',
                        'placeholder' => 'https://smartlink.example.com/offer/xxx',
                        'value' => $settings['final_ad_url']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('URL to open in new tab when user clicks confirm.') ?></span>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2"><?= __('Delay (seconds) before redirect') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['final_ad_delay_seconds']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 1,
                        'max' => 60,
                        'value' => $settings['final_ad_delay_seconds']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('Seconds to wait before redirecting to the original link.') ?></span>
                </div>
            </div>
            <?php endif; ?>

            <?php if (isset($settings['adsterra_social_bar'], $settings['adsterra_popunder'], $settings['adsterra_direct_link'])) : ?>
            <legend><?= __('Adsterra Integration') ?></legend>
            <p><?= __('Insert Adsterra scripts (Social Bar, Popunder, Direct Link) into ad interaction pages to optimize eCPM.') ?></p>
            <div class="row">
                <div class="col-sm-2"><?= __('Social Bar Script') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['adsterra_social_bar']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'textarea',
                        'rows' => 4,
                        'value' => $settings['adsterra_social_bar']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('Shown on banner/interstitial/captcha pages.') ?></span>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2"><?= __('Popunder Script') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['adsterra_popunder']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'textarea',
                        'rows' => 4,
                        'value' => $settings['adsterra_popunder']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('Triggered when user clicks Continue button.') ?></span>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2"><?= __('Direct Link Script') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['adsterra_direct_link']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'textarea',
                        'rows' => 4,
                        'value' => $settings['adsterra_direct_link']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('Alternative to Final Ad URL - use script instead of redirect.') ?></span>
                </div>
            </div>
            <?php endif; ?>
        ...
        <div role="tabpanel" id="security" class="tab-pane fade in">
            ...
            <legend><?= __('ProxyCheck.io - Anti VPN/Proxy') ?></legend>
            <p><?= __('Block visitors using VPN or proxy. Get free API key at <a href="https://proxycheck.io" target="_blank">proxycheck.io</a>') ?></p>

            <?php if (isset($settings['enable_proxycheck'], $settings['proxycheck_api_key'], $settings['proxycheck_block_url'])) : ?>
            <div class="row">
                <div class="col-sm-2"><?= __('Enable ProxyCheck') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['enable_proxycheck']['id'] . '.value', [
                        'label' => false,
                        'options' => ['yes' => __('Yes'), 'no' => __('No')],
                        'value' => $settings['enable_proxycheck']['value'],
                        'class' => 'form-control'
                    ]);
                    ?>
                </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('ProxyCheck API Key') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['proxycheck_api_key']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'text',
                        'value' => $settings['proxycheck_api_key']['value'],
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Block Redirect URL') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['proxycheck_block_url']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'text',
                        'value' => $settings['proxycheck_block_url']['value'],
                        'placeholder' => 'https://example.com/blocked'
                    ]);
                    ?>
                    <span class="help-block"><?= __('Optional. Redirect blocked visitors to this URL. Leave empty to show default blocked page.') ?></span>
                </div>
            </div>
            <?php if (isset($settings['proxy_check_provider'], $settings['ipinfo_token'])) : ?>
            <div class="row">
                <div class="col-sm-2"><?= __('Provider') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['proxy_check_provider']['id'] . '.value', [
                        'label' => false,
                        'options' => ['proxycheck' => 'ProxyCheck.io', 'ipinfo' => 'IPinfo.io'],
                        'value' => $settings['proxy_check_provider']['value'],
                        'class' => 'form-control'
                    ]);
                    ?>
                    <span class="help-block"><?= __('IPinfo.io fallback: requires token from <a href="https://ipinfo.io" target="_blank">ipinfo.io</a>. Use "org" keyword detection or Privacy API (paid).') ?></span>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2"><?= __('IPinfo Token') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['ipinfo_token']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'text',
                        'value' => $settings['ipinfo_token']['value'],
                        'placeholder' => __('Required when Provider = IPinfo.io')
                    ]);
                    ?>
                </div>
            </div>
            ...
            <?php if (isset($settings['campaign_verify_retries'])) : ?>
            <legend><?= __('Campaign Verification Hardening') ?></legend>
            <p><?= __('Configure website verification request timeout/retry behavior and default cron batch sizes.') ?></p>

            <div class="row">
                <div class="col-sm-2"><?= __('Retry Attempts') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['campaign_verify_retries']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 1,
                        'max' => 10,
                        'value' => $settings['campaign_verify_retries']['value']
                    ]);
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Connect Timeout (seconds)') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['campaign_verify_connect_timeout']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 1,
                        'max' => 60,
                        'value' => $settings['campaign_verify_connect_timeout']['value']
                    ]);
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Request Timeout (seconds)') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['campaign_verify_request_timeout']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 2,
                        'max' => 120,
                        'value' => $settings['campaign_verify_request_timeout']['value']
                    ]);
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Retry Delay (ms)') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['campaign_verify_retry_delay_ms']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 0,
                        'max' => 10000,
                        'value' => $settings['campaign_verify_retry_delay_ms']['value']
                    ]);
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Daily Recheck Batch Limit') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['campaign_verify_recheck_limit']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 1,
                        'max' => 5000,
                        'value' => $settings['campaign_verify_recheck_limit']['value']
                    ]);
                    ?>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-2"><?= __('Recheck Interval (days)') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['campaign_verify_recheck_days']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 0,
                        'max' => 30,
                        'value' => $settings['campaign_verify_recheck_days']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('Cron command uses these defaults: php bin/cake.php campaign_verification_recheck') ?></span>
                </div>
            </div>
            <?php endif; ?>

            <legend><?= __('Rate Limiting') ?></legend>
            <p><?= __('Max paid views per IP per link per day. Reset at midnight (site timezone).') ?></p>

            <?php if (isset($settings['rate_limit_views_per_link_day'])) : ?>
            <div class="row">
                <div class="col-sm-2"><?= __('Max Paid Views Per Link Per Day') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['rate_limit_views_per_link_day']['id'] . '.value', [
                        'label' => false,
                        'class' => 'form-control',
                        'type' => 'number',
                        'min' => 1,
                        'step' => 1,
                        'value' => $settings['rate_limit_views_per_link_day']['value']
                    ]);
                    ?>
                    <span class="help-block"><?= __('1 IP can earn from max this many views per link per day. Views beyond this still redirect but do not count for earnings.') ?></span>
                </div>
            </div>
            <?php endif; ?>

            <legend><?= __('Anti-Bypass') ?></legend>
            <?php if (isset($settings['anti_bypass_redirect_token'], $settings['anti_bypass_redirect_delay_min'], $settings['anti_bypass_redirect_delay_max'])) : ?>
            <div class="row">
                <div class="col-sm-2"><?= __('Token-based Redirect') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['anti_bypass_redirect_token']['id'] . '.value', [
                        'label' => false,
                        'options' => ['yes' => __('Yes'), 'no' => __('No')],
                        'value' => $settings['anti_bypass_redirect_token']['value'],
                        'class' => 'form-control'
                    ]);
                    ?>
                    <span class="help-block"><?= __('Hide redirect URL from extensions. Uses one-time token.') ?></span>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-2"><?= __('Redirect Delay (seconds)') ?></div>
                <div class="col-sm-10">
                    <span><?= __('Min') ?></span>
                    <?= $this->Form->input('Options.' . $settings['anti_bypass_redirect_delay_min']['id'] . '.value', ['label' => false, 'class' => 'form-control', 'type' => 'number', 'min' => 0, 'style' => 'display:inline-block;width:80px;', 'value' => $settings['anti_bypass_redirect_delay_min']['value']]); ?>
                    -
                    <span><?= __('Max') ?></span>
                    <?= $this->Form->input('Options.' . $settings['anti_bypass_redirect_delay_max']['id'] . '.value', ['label' => false, 'class' => 'form-control', 'type' => 'number', 'min' => 0, 'style' => 'display:inline-block;width:80px;', 'value' => $settings['anti_bypass_redirect_delay_max']['value']]); ?>
                    <span class="help-block"><?= __('Random delay before redirect (anti-automation).') ?></span>
                </div>
            </div>
            <?php endif; ?>
            <?php if (isset($settings['enable_anti_bypass'])) : ?>
            <div class="row">
                <div class="col-sm-2"><?= __('Enable Anti-Bypass') ?></div>
                <div class="col-sm-10">
                    <?=
                    $this->Form->input('Options.' . $settings['enable_anti_bypass']['id'] . '.value', [
                        'label' => false,
                        'options' => ['yes' => __('Yes'), 'no' => __('No')],
                        'value' => $settings['enable_anti_bypass']['value'],
                        'class' => 'form-control'
                    ]);
                    ?>
                    <span class="help-block"><?= __('Disable right-click, view source shortcuts on interstitial/banner pages.') ?></span>
                </div>
            </div>
            <?php endif; ?>
