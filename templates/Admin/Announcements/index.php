        ...
        <table class="table table-hover table-striped">
            ...
            <?php foreach ($announcements as $announcement): ?>
                <tr>
                    ...
                    <td>
                        <?= $this->Html->link($announcement->title, ['action' => 'edit', $announcement->id, '?' => ['lang' => get_option('language', 'en_US')] ]); ?> <?= $this->Html->link(get_option('language', 'en_US'), ['action' => 'edit', $announcement->id, '?' => ['lang' => get_option('language', 'en_US')] ], ['class' => 'label label-primary']); ?><br>
                        <p>
                        <?php foreach ( get_site_languages() as $lang ) : ?>
                        <?= $this->Html->link($lang, ['action' => 'edit', $announcement->id, '?' => ['lang' => $lang] ], ['class' => 'label label-default']); ?>
                        <?php endforeach; ?>
                        </p>
                    </td>
                    ...
                    <td>
                        <?= $this->Html->link(__('View'), ['action' => 'view', $announcement->slug, 'prefix' => false], ['class' => 'btn btn-primary btn-xs', 'target' => '_blank']); ?>
                        
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $announcement->id], ['confirm' => __('Are you sure?'), 'class' => 'btn btn-danger btn-xs']);
