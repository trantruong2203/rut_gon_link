...
<div class="box box-primary">
    <div class="box-body">
        ...
        <legend><?= __('KYC / Traffic') ?></legend>
        <table class="table table-striped table-hover">
            <tr>
                <td><?= __('Traffic Source') ?> (<?= __('Nguồn lưu lượng') ?>)</td>
                <td>
                    <?php
                    $sourceLabels = [
                    ...
                    if (!empty($sources)):
                        foreach ($sources as $src):
                            $src = is_array($src) ? $src : ['type' => 'other', 'url' => $src];
                            $label = $sourceLabels[$src['type'] ?? ''] ?? ($src['type'] ?? __('other'));
                            $url = $src['url'] ?? '';
                            if ($url): ?>
                                <div><?= h($label) ?>: <a href="<?= h($url) ?>" target="_blank" rel="noopener"><?= h($url) ?></a></div>
                            <?php endif;
                        endforeach;
                    else: ?>
                        <?= __('Not set') ?>
            ...
            <tr>
                <td><?= __('KYC Status') ?></td>
                <td><?= h($user->kyc_status ?: 'pending') ?></td>
            </tr>
        </table>
